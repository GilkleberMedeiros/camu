...
function getOriginOrPlaceholder(placeholder: string = "[origin]") 
{
  return typeof window !== "undefined" ? window.location.origin : placeholder;
}
...
whatsUrl.searchParams.append("text", `Conheça a ${siteName} em ${getOriginOrPlaceholder()}`);
...
facebookUrl.searchParams.append("u", `${getOriginOrPlaceholder()}`);
